...
```{r plot, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}

suppressPackageStartupMessages(library(tidyverse))
library(readr)

foodwaste <- read_csv("~/Desktop/MA415/ma4615-fa22-final-project-the_a_team/dataset/FAOSTAT Food waste & loss.csv")
...
foodwaste_new <- foodwaste %>% 
  group_by(year) %>%
  group_by(commodity) %>%
  summarize(year, commodity, loss_percentage)

foodwaste_new %>% 
  ggplot(aes(loss_percentage, commodity)) + 
  geom_boxplot()

```

![food_waste: figure_1](~/Desktop/MA415/ma4615-fa22-final-project-the_a_team/static/images/Screen Shot 2022-11-04 at 12.42.50 PM.jpeg)
