...
|           Hence, in this second model, we expanded upon the original simple regression model into a multi regression model including other macro variables that we had originally been interested about: 'Year', 'GDP', 'Agriculture as Percentage of GDP', 'Percentage of Land used for Agriculture', 'Population', and 'Production'. 

```{r echo=FALSE}
model2 <- lm(Loss ~ Year + GDP + AgriGDP + AgriLand + Population + Production, data = AggData)
summary(model2)
```
|           However, upon expanding our model, based on the p-value, it appears that only three out of six variables
