...
|           However, using these flaws we have provided potential avenues to further explore this topic. For example, instead of GDP, it was suggested that perhaps income may be a better and more significant predictor of food waste. And with income levels, this will allow for an even more detailed analysis as income can vary greatly even by county, thus, allowing the model to zoom in and out, from city to country. Additionally, while we examined many variables and their interaction terms, we, however, did not transform them. Therefore, another potential change to our analysis could be to log transform variables like GDP to see if there exists better correlation. 
